        ...
        return $form
            ->schema([
                ...
                Forms\Components\TextInput::make('cms_post.slug')
                    ...
                    ->maxLength(255)
                    ->rules([
                        function (PostService $service, ?Partner $record): Closure {
                            return function (
                                string $attribute,
                                mixed $state,
                                Closure $fail
            ...
            ->actions([
                Tables\Actions\ActionGroup::make([
                        ...
                        Tables\Actions\ViewAction::make()
                            ->extraModalFooterActions([
                                Tables\Actions\Action::make('edit')
                                    ...
                                    ->hidden(
                                        fn(): bool =>
                                        !auth()->user()->can('Editar [CMS] Parceiros'),
                                    ),
                            ]),
                        Tables\Actions\EditAction::make()
                            ->mutateRecordDataUsing(
                                fn(PartnerService $service, Partner $record, array $data): array =>
                                $service->mutateRecordDataToEdit(partner: $record, data: $data)
                            )
                            ->mutateFormDataUsing(
                                fn(PartnerService $service, Partner $record, array $data): array =>
                                $service->mutateFormDataToEdit(partner: $record, data: $data)
                            )
                            ->after(
                    ...
                    Tables\Actions\DeleteAction::make()
                        ->label(__('Excluir'))
                        ->before(
                            fn(PartnerService $service, Tables\Actions\DeleteAction $action, Partner $record) =>
                            $service->preventDeleteIf(action: $action, partner: $record)
                        ),
                ])
                    ->label(__('Ações'))
            ...
            ->bulkActions([
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\DeleteBulkAction::make()
                        ->hidden(
                            fn(): bool =>
                            !auth()->user()->can('Deletar [CMS] Parceiros'),
                        ),
                ]),
            ])
            ->emptyStateActions([
    ...
    public static function getTableFilters(): array
    {
        return [
            ...
            Tables\Filters\Filter::make('cmsPost.created_at')
                ...
                ->indicateUsing(
                    fn(PartnerService $service, mixed $state): ?string =>
                        ...
                        Infolists\Components\Tabs\Tab::make(__('Mídias'))
                            ->schema([
                                ...
                                Infolists\Components\TextEntry::make('cmsPost.embed_video')
                                    ...
                                    ->state(
                                        function (Partner $record): ?string {
