...
use App\Filament\Resources\Cms\StoryResource\Pages;
use App\Filament\Resources\Cms\StoryResource\RelationManagers;
        ...
        return $form
            ->schema([
                ...
                Forms\Components\TextInput::make('cms_post.slug')
                    ...
                    ->maxLength(255)
                    ->rules([
                        function (PostService $service, ?Story $record): Closure {
                            return function (
                                string $attribute,
                                mixed $state,
                                Closure $fail
                ...
                Forms\Components\TextInput::make('cms_post.embed_video')
                    ...
                    ->required(
                        fn(Get $get): bool =>
                        in_array($get('role'), [2]) && empty($get('video')), // 2 - Vídeo
                    )
                    ->maxLength(255)
                    ->live(onBlur: true)
                    ->hidden(
                ...
                Forms\Components\SpatieMediaLibraryFileUpload::make('video')
                    ...
                    ->required(
                        fn(Get $get): bool =>
                        in_array($get('role'), [2]) && empty($get('cms_post.embed_video')), // 2 - Vídeo
                    )
                    ->maxSize(15360)
            ...
            ->actions([
                Tables\Actions\ActionGroup::make([
                        ...
                        Tables\Actions\ViewAction::make()
                            ->extraModalFooterActions([
                                Tables\Actions\Action::make('edit')
                                    ...
                                    ->hidden(
                                        fn(): bool =>
                                        !auth()->user()->can('Editar [CMS] Histórias'),
                                    ),
                            ]),
                        Tables\Actions\EditAction::make()
                            ->mutateRecordDataUsing(
                                fn(StoryService $service, Story $record, array $data): array =>
                                $service->mutateRecordDataToEdit(story: $record, data: $data)
                            )
                            ->mutateFormDataUsing(
                                fn(StoryService $service, Story $record, array $data): array =>
                                $service->mutateFormDataToEdit(story: $record, data: $data)
                            )
                            ->after(
                    ...
                    Tables\Actions\DeleteAction::make()
                        ->label(__('Excluir'))
                        ->before(
                            fn(StoryService $service, Tables\Actions\DeleteAction $action, Story $record) =>
                            $service->preventDeleteIf(action: $action, story: $record)
                        ),
                ])
                    ->label(__('Ações'))
            ...
            ->bulkActions([
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\DeleteBulkAction::make()
                        ->hidden(
                            fn(): bool =>
                            !auth()->user()->can('Deletar [CMS] Histórias'),
                        ),
                ]),
            ])
            ->emptyStateActions([
    ...
    public static function getTableColumns(): array
    {
        return [
            ...
            Tables\Columns\TextColumn::make('role')
                ...
                ->badge()
                ->searchable(
                    query: fn(StoryService $service, Builder $query, string $search): Builder =>
                    $service->tableSearchByRole(query: $query, search: $search)
                )
                ->sortable(
                    query: fn(StoryService $service, Builder $query, string $direction): Builder =>
    ...
    public static function getTableFilters(): array
    {
        return [
            ...
            Tables\Filters\Filter::make('cmsPost.created_at')
                ...
                ->indicateUsing(
                    fn(StoryService $service, mixed $state): ?string =>
