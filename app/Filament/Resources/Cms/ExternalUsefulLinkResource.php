        ...
        return $form
            ->schema([
                Forms\Components\Select::make('role')
                    ...
                    ->options(ExternalUsefulLinkRoleEnum::class)
                    ->default(1)
                ...
                Forms\Components\TextInput::make('cms_post.slug')
                    ...
                    ->maxLength(255)
                    ->rules([
                        function (PostService $service, ?ExternalUsefulLink $record): Closure {
                            return function (
                                string $attribute,
                                mixed $state,
                                Closure $fail
            ...
            ->actions([
                Tables\Actions\ActionGroup::make([
                        ...
                        Tables\Actions\ViewAction::make()
                            ->extraModalFooterActions([
                                Tables\Actions\Action::make('edit')
                                    ...
                                    ->button()
                                    ->url(
                                        fn(ExternalUsefulLink $record): string =>
                                        self::getUrl('edit', ['record' => $record]),
                                    )
                                    ->hidden(
                                        fn(): bool =>
                                        !auth()->user()->can('Editar [CMS] Links Externos'),
                                    ),
                            ]),
                        Tables\Actions\EditAction::make()
                            ->mutateRecordDataUsing(
                                fn(ExternalUsefulLinkService $service, ExternalUsefulLink $record, array $data): array =>
                                $service->mutateRecordDataToEdit(externalUsefulLink: $record, data: $data)
                            )
                            ->mutateFormDataUsing(
                                fn(ExternalUsefulLinkService $service, ExternalUsefulLink $record, array $data): array =>
                                $service->mutateFormDataToEdit(externalUsefulLink: $record, data: $data)
                            )
                            ->after(
                    ...
                    Tables\Actions\DeleteAction::make()
                        ->label(__('Excluir'))
                        ->before(
                            fn(ExternalUsefulLinkService $service, Tables\Actions\DeleteAction $action, ExternalUsefulLink $record) =>
                            $service->preventDeleteIf(action: $action, externalUsefulLink: $record)
                        ),
                ])
                    ->label(__('Ações'))
            ...
            ->bulkActions([
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\DeleteBulkAction::make()
                        ->hidden(
                            fn(): bool =>
                            !auth()->user()->can('Deletar [CMS] Links Externos'),
                        ),
                ]),
            ])
            ->emptyStateActions([
    ...
    public static function getTableColumns(): array
    {
        return [
            ...
            Tables\Columns\TextColumn::make('role')
                ...
                ->badge()
                ->searchable(
                    query: fn(ExternalUsefulLinkService $service, Builder $query, string $search): Builder =>
                    $service->tableSearchByRole(query: $query, search: $search)
                )
                ->sortable(
                    query: fn(ExternalUsefulLinkService $service, Builder $query, string $direction): Builder =>
    ...
    public static function getTableFilters(): array
    {
        return [
            ...
            Tables\Filters\Filter::make('cmsPost.created_at')
                ...
                ->indicateUsing(
                    fn(ExternalUsefulLinkService $service, mixed $state): ?string =>
                ...
                Infolists\Components\Tabs::make('Label')
                    ->tabs([
                        ...
                        Infolists\Components\Tabs\Tab::make(__('Conteúdo e Tags'))
                            ...
                            ->visible(
                                fn(ExternalUsefulLink $record): bool =>
                        ...
                        Infolists\Components\Tabs\Tab::make(__('Mídias'))
                            ->schema([
                                ...
                                Infolists\Components\TextEntry::make('cmsPost.embed_video')
                                    ...
                                    ->state(
                                        function (ExternalUsefulLink $record): ?string {
