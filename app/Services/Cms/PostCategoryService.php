<?php

namespace App\Services\Cms;

use App\Enums\DefaultStatusEnum;
use App\Models\Cms\PostCategory;
use App\Services\BaseService;
use Filament\Notifications\Notification;
use Illuminate\Contracts\Database\Eloquent\Builder;
use Filament\Forms;
use Filament\Forms\Set;
use Illuminate\Database\Eloquent\Collection;
use Illuminate\Support\Str;

class PostCategoryService extends BaseService
{
    public function __construct(protected PostCategory $postCategory)
    {
        parent::__construct();
    }

    public function getOptionsByPostCategories(): array
    {
        return $this->postCategory->byStatuses(statuses: [1]) // 1 - Ativo
            ->pluck('name', 'id')
            ->toArray();
    }

    public function getQueryByPostCategories(Builder $query): Builder
    {
        return $query->byStatuses(statuses: [1]); // 1 - Ativo
    }

    public function quickCreateActionByPostCategories(
        string $field,
        bool $multiple = false
    ): Forms\Components\Actions\Action {
        return Forms\Components\Actions\Action::make($field)
            ->label(__('Criar Categoria'))
            ->icon('heroicon-o-plus')
            ->form([
                Forms\Components\Grid::make(['default' => 2])
                    ->schema([
                        Forms\Components\TextInput::make('name')
                            ->label(__('Nome'))
                            ->required()
                            ->minLength(2)
                            ->maxLength(255)
                            ->columnSpanFull(),
                    ]),
            ])
            ->action(
                function (array $data, Set $set, mixed $state) use ($field, $multiple): void {
                    $category = $this->postCategory->create($data);

                    if ($multiple) {
                        $values = is_array($state) ? $state : [];
                        $set($field, array_merge($values, [$category->id]));
                    } else {
                        $set($field, $category->id);
                    }
                }
            );
    }

    /**
     * $action can be:
     * Filament\Tables\Actions\DeleteAction;
     * Filament\Actions\DeleteAction;
     */
    public function preventDeleteIf($action, PostCategory $postCategory): void
    {
        $title = __('Ação proibida: Exclusão de categoria');

        if ($this->isAssignedToPost(postCategory: $postCategory)) {
            Notification::make()
                ->title($title)
                ->warning()
                ->body(__('Esta categoria possui postagens associadas. Para excluir, você deve primeiro desvincular todas as postagens que estão associados a ela.'))
                ->send();

            $action->halt();
        }
    }

    public function deleteBulkAction(Collection $records): void
    {
        $blocked = [];
        $allowed = [];

        foreach ($records as $postCategory) {
            if ($this->isAssignedToPost(postCategory: $postCategory)) {
                $blocked[] = $postCategory->name;
                continue;
            }

            $allowed[] = $postCategory;
        }

        if (!empty($blocked)) {
            $displayBlocked = array_slice($blocked, 0, 5);
            $extraCount = count($blocked) - 5;

            $message = __('As seguintes categorias não podem ser excluídas: ') . implode(', ', $displayBlocked);

            if ($extraCount > 0) {
                $message .= " ... (+$extraCount " . __('outros') . ")";
            }

            Notification::make()
                ->title(__('Algumas categorias não puderam ser excluídas'))
                ->warning()
                ->body($message)
                ->send();
        }

        collect($allowed)->each->delete();

        if (!empty($allowed)) {
            Notification::make()
                ->title(__('Excluído'))
                ->success()
                ->send();
        }
    }

    protected function isAssignedToPost(PostCategory $postCategory): bool
    {
        return $postCategory->cmsPosts()
            ->exists();
    }
}
